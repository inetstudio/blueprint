...
EXAMPLE
        ...
        $releaseData = array_filter(
            [
                ...
                'refs' => $config['refs'],
                'ref' => trim(run('git rev-parse HEAD')),
                'url' => $config['url'],
    ...
    return static function ($config = []): array {
        ...
        return array_map(
            static function (string $line): array {
                ...
                $message = trim(run('git show -s --format=%B '.$ref));

                $patch_set = [];

                foreach (['A', 'M', 'D'] as $type) {
                    $files = trim(run('git show --diff-filter='.$type.' --pretty="format:" --name-only '.$ref));
                ...
                return [
                    ...
                    'message' => $message,
                    'patch_set' => $patch_set,
                    'repository' => get('sentry_repository_name', '')
                ];
            },
            $lines
        );
